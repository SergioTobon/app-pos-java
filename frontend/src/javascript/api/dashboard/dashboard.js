...
document.addEventListener('DOMContentLoaded', function() {
    // Asegúrate de que esta es la única llamada para el dashboard
    // o que se integra en tu función de inicialización principal (ej. initComprasCRUD)
    cargarDashboardMetrics();
    renderTopSellingProductsChart();
    renderMostFrequentClientsChart();

    // Si tienes una función initComprasCRUD, y quieres que estas se ejecuten después de ella,
    // asegúrate de llamarlas dentro de initComprasCRUD o después de su finalización.
    // Ejemplo (si initComprasCRUD es async):
    // if (typeof initComprasCRUD === 'function') {
    //     initComprasCRUD().then(() => {
    //         cargarDashboardMetrics();
    //         renderTopSellingProductsChart();
    //         renderMostFrequentClientsChart();
    //     }).catch(error => console.error("Error en initComprasCRUD:", error));
    // }
});